...
execute at @s as @e[tag=tv_stand] if score @s equal_rot matches 0 if score @s tv_on matches 1 run tellraw @p {"text": "Your TV is now off!", "color": "red"}
execute at @s as @e[tag=tv_stand] if score @s equal_rot matches 0 run scoreboard players set @s tv_on 0